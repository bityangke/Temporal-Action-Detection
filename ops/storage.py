...
class RolloutStorage(object):
    def __init__(self, num_steps, num_processes, obs_shape, action_space, state_size=1):
        ...
        self.labels = torch.zeros(num_steps + 1, num_processes, 1)
    ...
    def insert(self, current_obs, state, action, action_log_prob, value_pred, reward, mask, label):
        ...
        self.labels[self.step + 1].copy_(label)
    ...
    def after_update(self):
        ...
        self.labels[0].copy_(self.labels[-1])
    ...
    def clf_generator(self, num_mini_batch):
        num_steps, num_processes = self.rewards.size()[0:2]
        batch_size = num_processes * num_steps
        assert batch_size >= num_mini_batch, (
            f"PPO requires the number processes ({num_processes}) "
        ...
        for indices in sampler:
            observations_batch = self.observations[:-1].view(-1,
                                        *self.observations.size()[2:])[indices]
            label = self.labels.view(-1)[indices]
            # old_action_log_probs_batch = self.action_log_probs.view(-1, 1)[indices]
            # adv_targ = advantages.view(-1, 1)[indices]

            yield observations_batch, label
